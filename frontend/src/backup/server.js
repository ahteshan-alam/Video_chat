...
io.on("connection", (socket) => {
    ...
    socket.on("disconnect", () => {
        let partner
        rooms[socket.room].forEach(client=>{if(client.id===socket.id) {partner=client.partner; client.partner=null }})
        rooms[socket.room].forEach(client=>{if(client.id===partner) client.busy=false})
